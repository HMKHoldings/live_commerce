  ...
  return <main className="product-detail wrap">
    ...
    <section className="pd-overview" aria-labelledby="pd-title">
      ...
      <div className="pd-summary">
        ...
        <dl className="pd-facts"><dt>배송비</dt><dd>무료배송 · 데모 기준</dd><dt>상품분류</dt><dd>{product.category}</dd><dt>상품구성</dt><dd>{details.option || product.name}</dd><dt>원산지</dt><dd>판매자 정보 등록 예정</dd><dt>상품코드</dt><dd>OS-{String(product.id).padStart(4, "0")}</dd></dl>
    ...
    <section id="pd-info" className="pd-section">{tabs("info")}
      ...
      <div id="pd-specifications" hidden={!expanded}><dl className="pd-specs"><dt>상품명</dt><dd>{product.name}</dd><dt>구성</dt><dd>{details.option || product.name}</dd><dt>판매가격</dt><dd>{won(product.price)}</dd><dt>제조사 / 원산지</dt><dd>판매자 정보 등록 예정</dd><dt>재질 / 성분 / 보관방법</dt><dd>판매자 상세정보 등록 예정</dd></dl></div>
    ...
    <section id="pd-questions" className="pd-section">{tabs("questions")}<h2 className="pd-sr-only">상품문의</h2>
      ...
      {questionOpen && <form className="pd-form" onSubmit={event => { event.preventDefault(); const data = new FormData(event.currentTarget); const body = String(data.get("body")).trim(); if (!body) return; setQuestions(items => [...items, { body, date: new Date().toLocaleDateString("ko-KR") }]); setQuestionOpen(false); }}><label>상품 문의<textarea name="body" required maxLength={1000} rows={4} /></label><p>데모 문의는 이 페이지에서만 표시되며 판매자에게 전송되지 않습니다.</p><button className="pd-buy">문의 등록</button></form>}
    </section>

    <section id="pd-shipping" className="pd-section">{tabs("shipping")}<div className="pd-shipping"><h2>배송 / 교환 / 반품 안내</h2><dl className="pd-specs"><dt>배송 안내</dt><dd>배송비는 현재 데모 기준 무료입니다. 배송 일정과 추가 지역 배송비는 판매자 등록 후 안내됩니다.</dd><dt>교환 / 반품</dt><dd>접수 방법, 반품 주소와 비용은 판매자 정책 등록 후 안내됩니다.</dd><dt>문의 안내</dt><dd>상품 관련 문의는 위의 상품문의에서 확인할 수 있습니다.</dd></dl></div></section>
    ...
    <section id="pd-reviews" className="pd-section">{tabs("reviews")}<div className="pd-review-summary"><div><h2>상품평</h2><span className="pd-stars">{rating ? "★★★★★" : "☆☆☆☆☆"}</span><p>({productReviews.length}개 리뷰)</p></div><strong>{rating ? `${rating}점` : "—"}</strong><img src={photo} alt="" loading="lazy" /></div>
      ...
      {reviewOpen && <form className="pd-form" onSubmit={event => { event.preventDefault(); const data = new FormData(event.currentTarget); if (!String(data.get("title")).trim() || !String(data.get("body")).trim()) return; setWrittenReviews(items => [...items, { title: String(data.get("title")).trim(), body: String(data.get("body")).trim(), rating: Number(data.get("rating")), author: "나 (데모)" }]); setReviewOpen(false); }}><label>평점<select name="rating">{[5,4,3,2,1].map(value => <option key={value} value={value}>{value}점</option>)}</select></label><label>제목<input name="title" required maxLength={100} /></label><label>내용<textarea name="body" required rows={4} maxLength={1000} /></label><p>데모 리뷰는 새로고침하거나 다른 상품으로 이동하면 초기화됩니다.</p><button className="pd-buy">상품평 등록</button></form>}
    </section>
  </main>;
}
