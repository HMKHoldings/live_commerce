...
export default function ProductCatalog({ collection, onSelect, onBack }) {
  ...
  const visibleProducts = collection.products.filter(product => !categorized || category === "전체" || accepted.includes(String(product.category).toLowerCase()));
  return <main className={`product-catalog wrap${categorized ? " best-catalog" : ""}`}>
    <button className="catalog-back" onClick={onBack}><ChevronLeft size={18} /> 돌아가기</button>
        <div className="catalog-heading"><h1>{collection.title}</h1>{!arrivals && <p>{ranked ? "오렌지스토어 베스트 상품을 만나보세요." : collection.description || "다양한 상품과 특별한 혜택을 만나보세요."}</p>}</div>
    {categorized && <div className="best-category-tabs" aria-label={`${collection.title} 카테고리`}>{categories.map(([label]) => <button key={label} type="button" aria-pressed={category === label} onClick={() => setCategory(label)}>{label}</button>)}</div>}
