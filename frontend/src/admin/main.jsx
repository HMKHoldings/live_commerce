...
import {adminViews,rowsForView} from './viewConfig';
...
const statusLabels={draft:'임시저장',published:'게시중',hidden:'숨김',pending:'대기중',approved:'승인',rejected:'반려',new:'신규',processing:'처리중',shipped:'배송중',completed:'완료',cancelled:'취소'};
...
function Admin(){
  ...
  const [user,setUser]=useState(null),[checking,setChecking]=useState(true),[section,setSection]=useState('dashboard'),[view,setView]=useState('dashboard'),[viewTitle,setViewTitle]=useState('');
  ...
  const load=async()=>{
    ...
    try{
      if(section==='dashboard'){
        const [products,orders,reviews,questions,activity,notices]=await Promise.all(['products','orders','reviews','questions','audit','notices'].map(key=>api('/admin/'+key)));
        if(request!==loadId.current)return;
        setStats({products,orders,reviews,questions,activity,notices});setRows(orders);
  ...
  const remove=async(record,skipConfirm=false)=>{
    if(!skipConfirm&&!confirm(`“${record.name||record.title||record.id}” 항목을 삭제하시겠습니까?`))return;
  ...
  const saveSiteSettings=async next=>{setBusy(true);setError('');try{await api(`/admin/settings/${next.id||'site'}`,{method:'PUT',body:next});announce('사이트 설정이 저장되었습니다.');await load();}catch(e){setError(e.message);throw e;}finally{setBusy(false);}};
  const uploadSiteAsset=async file=>{setError('');try{const result=await api('/admin/upload',{method:'POST',body:file,raw:true});return result.url;}catch(e){setError(e.message);throw e;}};
  ...
  const viewConfig=adminViews[view]||{};
  const visible=rowsForView(loadedSection===section?rows:[],viewConfig).filter(row=>(!filter||row.status===filter)&&JSON.stringify(row).toLowerCase().includes(search.toLowerCase()));
  const title=viewTitle||modules.find(([key])=>key===section)?.[1];
  const pageDescription=section==='audit'?'관리자 작업과 콘텐츠 변경 이력을 안전하게 확인합니다.':view==='6:2'?'신규, 재구매, VIP 고객과 구매 행동을 한눈에 확인하세요.':view==='6:1'?'상품별 매출, 판매수량, 재고와 카테고리 현황을 한눈에 확인하세요.':view==='6:0'?'주문 데이터를 기준으로 기간별 매출과 주문 상태를 분석합니다.':'내용을 수정하고 저장하면 연결된 스토어에 반영됩니다.';
  return <div className={'admin-shell '+(menuOpen?'menu-open':'')}><AdminSidebar section={section} navigate={navigate} /><div className="admin-workspace"><header><button className="admin-menu-toggle" aria-label="메뉴 열기 / 닫기" aria-expanded={menuOpen} onClick={()=>setMenuOpen(v=>!v)}><Menu size={21}/></button><form className="admin-global-search" onSubmit={e=>{e.preventDefault();const query=new FormData(e.currentTarget).get('query');navigate('products');setTimeout(()=>setSearch(query),0);}}><Search size={17}/><input name="query" aria-label="상품 검색" placeholder="상품명, 상품 ID를 검색하세요..."/><button type="submit">검색</button></form><div className="admin-account"><span className="admin-avatar">{user.username.slice(0,1).toUpperCase()}</span><span><b>{user.username}</b><small>관리자</small></span><button aria-label="로그아웃" onClick={async()=>{try{await api('/auth/logout',{method:'POST'});setCsrf('');setUser(null);}catch(e){setError(e.message);}}}><LogOut size={17}/></button></div></header><main><div className={section==='dashboard'?'admin-page-heading dashboard-shell-heading':'admin-page-heading'}><div><span className="admin-kicker">ORANGE STORE ADMIN</span><h1>{section==='dashboard'?'안녕하세요, 관리자님!':title}</h1><p>{section==='dashboard'?'오렌지스토어의 오늘 현황을 한눈에 확인해보세요.':pageDescription}</p></div><button onClick={load} disabled={busy}>새로고침</button></div>{error&&<div className="admin-error" role="alert">{error}</div>}{message&&<div className="admin-success" role="status">{message}<button aria-label="알림 닫기" onClick={()=>setMessage('')}>×</button></div>}
    {section==='dashboard'&&(loadedSection===section?<Dashboard data={stats} navigate={navigate}/>:<p className="admin-loading">현황을 불러오는 중…</p>)}
    {section==='products'&&(view==='products'||view==='0:0')&&<ProductList rows={loadedSection===section?rows:[]} loaded={loadedSection===section} busy={busy} onCreate={()=>{setIsNew(true);setEditor({...defaults.products,id:''});setError('');}} onEdit={row=>{setIsNew(false);setEditor(structuredClone(row));setError('');}} onDelete={remove} onNavigate={navigate}/>}
    {section==='products'&&view==='0:3'&&<BrandManager products={loadedSection===section?rows:[]} loaded={loadedSection===section} onEdit={row=>{setIsNew(false);setEditor(structuredClone(row));setError('');}} onBrandChange={async(items,nextBrand)=>{setBusy(true);setError('');try{for(const item of items)await api(`/admin/products/${item.id}`,{method:'PUT',body:{...item,brandName:nextBrand}});announce(nextBrand?'브랜드 이름이 변경되었습니다.':'브랜드가 삭제되고 상품 연결이 해제되었습니다.');await load();}catch(error){setError(error.message);throw error;}finally{setBusy(false);}}}/>}
    {section==='orders'&&(view==='orders'||view==='1:0')&&<OrderList orders={loadedSection===section?rows:[]} loaded={loadedSection===section} onEdit={row=>{setIsNew(false);setEditor(structuredClone(row));setError('');}}/>}
    {section==='settings'&&(view==='settings'||view==='7:0')&&<SiteSettings settings={rows.find(row=>row.id==='site')||rows[0]} loaded={loadedSection===section} busy={busy} onSave={saveSiteSettings} onUpload={uploadSiteAsset}/>}
    {section==='orders'&&view==='6:0'&&<SalesAnalytics orders={loadedSection===section?rows:[]} loaded={loadedSection===section}/>}
    ...
    {editor&&section!=='orders'&&<div className="admin-editor-backdrop"><section className="admin-editor" role="dialog" aria-modal="true" aria-labelledby="editor-title"><div className="admin-editor-heading"><div><small>{title}</small><h2 id="editor-title">{isNew?'새 항목 만들기':'내용 수정'}</h2></div><button aria-label="편집 닫기" onClick={()=>{if(confirm('편집을 닫을까요?'))setEditor(null);}}>×</button></div>{error&&<p className="admin-error" role="alert">{error}</p>}{section==='banners'&&<div className={'admin-banner-preview theme-'+(editor.theme||'fresh')} style={editor.image?{backgroundImage:`linear-gradient(90deg,rgba(0,0,0,.5),rgba(0,0,0,.05)),url("${assetPath(editor.image)}")`}:undefined}><span>현재 배너 미리보기</span><strong>{editor.title||editor.label||'메인 배너'}</strong><small>{editor.description||editor.eyebrow||'이미지를 선택하면 바로 미리보기에 표시됩니다.'}</small>{!editor.image&&<i>테마 기반 디자인</i>}</div>}<form onSubmit={save}><div className="admin-fields">{Object.entries({...defaults[section],...editor}).filter(([key])=>key!=='_version').map(([key,value])=>{
      ...
      if(key==='status')return <label key={key}>{labels[key]}<select value={value} onChange={e=>update(e.target.value)}>{(section==='orders'?['new','processing','shipped','completed','cancelled']:['reviews','questions'].includes(section)?['pending','approved','rejected']:['draft','published','hidden']).map(v=><option key={v} value={v}>{statusLabels[v]||v}</option>)}</select></label>;
